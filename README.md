...
Buses report true GPS positions and move on the map, drawn from the City zoom in, which
is where the map opens (below it the Buses button still counts them); subways are placed at their
...
The committed test fixture under `backend/tests/fixtures/njt_gtfs/` is generated
from the live archive, which needs credentials; nothing in CI touches the live API.

```bash
python backend/scripts/gen_njt_fixture.py   # mints once, downloads, trims, verifies
```

**This is the one feed in the repo that cannot be captured without an account,**
so its fixtures are generated by hand and committed. Until they are, the golden
tests that read them skip locally and **fail in CI** (see
`conftest.golden_fixture_guard`, which fails rather than skips under `CI=true`
precisely so dormant goldens cannot merge unnoticed). The generator fails loudly on
any drift from the probed facts, including a feed that starts shipping
`calendar.txt` or `feed_info.txt`. Eyeball the printed tables before committing,
per house rules.

The **realtime** capture (15b) is the same handoff and needs the static one first,
because the golden decodes against it:

```bash
python backend/scripts/gen_njt_rt_fixture.py   # one mint, both realtime feeds
```

**Run it at rush hour.** The probe's own words are that "the overnight numbers are
optimistic by roughly 2x", and the shapes these goldens exist to preserve only
appear under load: a trip-level CANCELED trip keeping full times on the stops it
marks SKIPPED (8% of peak Penn `stop_time_update`s), and both SKIPPED variants (238
with times, 35 without). An overnight capture is a valid feed with almost nothing in
it, and the generator refuses to write one: it checks the trap-shape counts, the
745-of-745 `entity.id`/`trip_short_name` agreement, and the header lag against the

...
**A FOURTH CONSUMER, and a fifth member (`claude/subway-hub-definition`).** The list of three
above was already short when this branch opened: MR5's popup kicker draws the routes calling at
a station as route plates, so an empty index is a kicker with no plates, a fourth rider-visible
loss nobody had written down. It is named now, in `_REQUIRED_MEMBERS`' comment and in
`load_subway_station_routes`' docstring. The same branch made a hub a station COMPLEX rather than
a stop (F9's row, amended), and the complexes come from `transfers.txt`, so that file joined the
required members by the same rule, with three consumers of its own: the transfer ring, the hub
label class and the kicker, which lists the complex's routes rather than the stop's.
`_REQUIRED_MEMBERS` is `("stops.txt", "shapes.txt", "trips.txt", "stop_times.txt",
"transfers.txt")`, and `transfers.txt` is the one of them with a content floor as well as a
presence check: on the operator's ruling, a table with no cross-stop row fails the load through
`require_parsed`, the gate `stops.txt` already has. The contract monitor's subway file list,
which had stayed `("stops.txt", "shapes.txt")` through this note's own branch, names all five
now and is held equal to the app's. The station alerts join is the one consumer of the routes index that did NOT
move to the complex: an alert for the 1 belongs on the 1's platform, and `subway.spec.js`
**D2i4** holds that on the 7's platform at Times Square, where the kicker now names the 1.
...
| **F9** | **`isTransferStation` counted route IDS, so skip-stop and local-express pairs were interchanges.** Marcy Av is J and Z, one line taking turns; every `["A","C"]` and `["4","5"]` stop is the same shape. All of them drew the paper transfer ring and took the `hub` class the zoom-12 band exists to keep sparse. | **The operator's ruling: count TRUNKS**, the groups `lineColor()` already knows, because sharing a colour is what being one line means. `P1f`'s subway station pin moves with it: six lines, both stations, ring to dot, nothing else in any pin. **AMENDED on `claude/subway-hub-definition`, after the label-band diagnosis of 2026-09-25.** Counting trunks was right; counting them PER STOP was not. Measured on production's own `/api/subway-stops`: 108 of the rule's 124 rings sat on stops `transfers.txt` joins to no other (the B beside the C on Central Park West, the D beside the R on Fourth Avenue), which is shared track, and 22 of the 35 real complexes had no ring at all, Times Square among them, because each of its five stops carries one trunk. The deployed build named 96 St, 86 St and 72 St at zoom 13 for exactly that reason, and the band, the toggle and the data were all correct. **The operator's second ruling: a hub is a station complex**, from `transfers.txt`'s cross-stop rows by union-find: a complex serving three or more trunks, or two or more trunks across two or more stops. 124 rings become 100 stops in 48 complexes (`subway.spec.js` **D2i1**, every one named), each complex's name drawn once on the stop whose id is the complex id (the operator's third ruling on this row, **D2i6**), the Key's ring row reading "Transfer station: change between lines here" (P1e moved by that wording), no local is named at zoom 13 (**D2i2**), a payload that names no complex still takes this row's per-stop answer, which is what a browser's cached copy is for the hour after a deploy (**D2i5**, review finding H1), and `P2a`'s registry pin gains the `complex` field it now carries. The record is `docs/reviews/subway-hub-definition/PR-BODY.md`. |
...
| (not stated) the map opens at zoom 12 | no bus is drawn at the opening view | The brief's own node test says 12 is hidden, and the map opens at 12. **For the operator:** a rider now lands on a map with no buses and a strip counting all of them, and meets the words only by hovering the button or opening the Key. If the opening view should draw buses, the smaller change is the opening zoom rather than the rule. |
...
| axe green with the buses undrawn at the opening view and at Rail and drawn at City, at 1280, 375 and 320, in both themes, with the bus seen per rule | `a11y.spec.js` A1w, three new states, eighteen scans |
...
| --- | --- | --- |
| M0 | none: the opening control, every gate once | **survived all ten gates**, as it must |
...
| M7b | the same, at the axe tier | A1w's City state |
...
| M13 | A4b measured at the opening zoom again | A4b, "bus-marker is not drawn here" |
| M14 | D4f read at the opening zoom again | D4f, through the `marks()` guard |
| M15 | D4g read at the opening zoom again | D4g, through the same guard |
| M16 | the rule as a re-render: bus markers taken off the layer below 13 | P6a |
| M17 | smoke 7 clicks a bus at the opening zoom again | smoke 7, the click never finding a drawn target |
...
| M20 | `theme.spec.js` reads its families at the opening zoom again (V4) | D5d, the bus family missing from the page |
| M21 | P4c measures at the opening zoom again (V4) | P4c, "every family is measured" |
...
| M25 | A7c clicks a bus at the opening zoom again (V10) | A7c, through the undrawn-target guard |
| M0z | none: the closing control, every gate again | **survived all ten gates**, as it must |

**Thirty-two rows at `2950451`: both controls survived every one of their ten gates, the other thirty died, none failed to run, no gate needed the clock-race retry, and every anchor matched exactly once.** Every death is on the assertion its row names. The table's first run, at `f2cd698` before the review, had twenty-three rows and the same shape of result: M0 survived and the other twenty-two died.
...
1. **The landing view draws no bus.** The map opens at zoom 12 and the rule, as briefed, hides buses
   below 13, so a rider lands on a strip counting every bus and a map drawing none
   (`after-open.png`). The words that explain it are in the Buses button's tooltip, which a phone
   never shows, and in the Key, which opens closed. If the landing view should draw buses, the
   smaller change is the opening zoom, not the rule.
...
| Gate | At the tip |
| --- | --- |
| `pytest` (backend) | **1738** passed. Run at `68d31da`; no backend file has changed on this branch at all |
| `ruff check`, `ruff format --check`, `mypy` | clean; 79 files formatted; 30 source files |
| contract-tier lint and format | clean |
| contract API tier | **38** passed (reads the backend only, unchanged) |
| contract browser tier, C6e1 to C6e5 | **5 of 5** at `2950451` |
| node tier | **400 of 400** at `2950451` (397 at the base plus this branch's 3), and 400 again in a worktree with no `node_modules`, which is the condition `frontend-tests` runs in |
| hermetic e2e | **341 of 347** at `2950451`, at load averages of 39 to 60 from other processes. The six: five pre-navigation clock races and A1v3's load timeout, all on the flake list. Re-run alone, four passed; D2w and A1v3 failed again; D2w then passed 1 of 1 and 4 of 4, and A1v3 fails at `d49e9a7` under the same load. 347 is the base's 318 plus P6's 3, D7's 8 and A1w's 18 |
| `run_all.sh` | **15 of 15** at `2950451` |
| the mutation table | 32 rows at `2950451`, as above |
